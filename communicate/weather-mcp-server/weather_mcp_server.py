...
from winerror import FACILITY_GAME
...
def get_weather_data(city: str) -> Dict[str, Any]:
    ...
    utl = f"https://wttr.in/{city_en}?format=j1"
    response = requests.get(utl, timeout=10)
...
def get_weather(city: str) -> str:
    ...
    # ensure_ascii=False：核心参数！保证中文、日文等非 ASCII 字符正常显示（比如不会把 “北京” 转成 \u5317\u4eac 这种 Unicode 编码）。
    # indent=2：格式化 JSON 字符串，每行缩进 2 个空格，让返回的字符串更易读（比如调试时能直观看到数据结构）。
