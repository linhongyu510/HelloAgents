...
class MySimpleAgent(SimpleAgent):
    ...
    def __init__(
        ...
        print(f"{name} 完成，工具调用：{'启用' if self.enable_tool_calling else '禁用'}")

    def run(self, input_text: str, max_tool_iteration: int = 3, **kwargs):
        """
        重写的运行方法，实现简单对话逻辑，支持可选工具调用
        :param input_text:
        :param max_tool_iteration:
        :param kwargs:
        :return:
        """
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        tools_section += "工具调用结果会自动插入到会话中，然后你可以基于结果继续回答。\n"
